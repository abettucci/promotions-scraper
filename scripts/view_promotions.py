...
def view_promotions(limit=None, supermarket=None, show_full=False):
    ...
    for idx, promo in enumerate(promotions, 1):
        ...
        # Exclusiones (mostrar completo con wrapping)
        if promo['exclusions']:
            exclusions_text = promo['exclusions']
            label = "  ⛔ Exclusiones:   "
            # Dividir en líneas de máximo 100 caracteres
            if len(exclusions_text) <= 100:
                print(f"{label}{exclusions_text}")
            else:
                # Imprimir primera línea
                print(f"{label}{exclusions_text[:100]}")
                # Imprimir líneas adicionales con indentación
                remaining = exclusions_text[100:]
                while remaining:
                    print(f"                    {remaining[:100]}")
                    remaining = remaining[100:]
        
        # Requisitos (mostrar completo con wrapping)
        if promo['requirements']:
            requirements_text = promo['requirements']
            label = "  ✅ Requisitos:    "
            if len(requirements_text) <= 100:
                print(f"{label}{requirements_text}")
            else:
                print(f"{label}{requirements_text[:100]}")
                remaining = requirements_text[100:]
                while remaining:
                    print(f"                    {remaining[:100]}")
                    remaining = remaining[100:]
        ...
        if promo['terms_raw']:
            if show_full:
                ...
                for i in range(0, len(terms_text), 100):
                    print(f"  {terms_text[i:i+100]}")
            else:
                # Mostrar las primeras 3 líneas (300 caracteres)
                terms_text = promo['terms_raw']
                print(f"\n  📋 Términos y Condiciones:")
                print(f"  {'-'*116}")
                if len(terms_text) <= 300:
                    for i in range(0, len(terms_text), 100):
                        print(f"  {terms_text[i:i+100]}")
                else:
                    # Primeras 3 líneas
                    for i in range(0, 300, 100):
                        print(f"  {terms_text[i:i+100]}")
                    print(f"  ... (usa opción 4 para ver completo)")
